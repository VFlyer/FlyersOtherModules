...
public class SimonsStagesScript : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        Module.OnActivate += delegate // Consistency reasons. TestHarness versus ingame. While it does not work on TestHarness, ingame can work otherwise and visa versa.
        {
            moduleCount = Bomb.GetSolvableModuleNames().Where(x => !ignoredModules.Contains(x)).Count();
            stagesToGenerate = moduleCount - 1;
        };
        moduleCount = 1; // Temporary until the lights turn on. 
        indicatorText.text = "";
        moduleLocked = true;
        foreach (LightInformation device in lightDevices)
        {
